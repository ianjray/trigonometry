...
RightAngledTriangle RightAngledTriangle::with_a_b_c(double a, double b, double c)
{
...
//@unittest clang++ -std=c++17 -DUNITTEST_RIGHTANGLEDTRIANGLE -Wall angle.cpp fcmp.cpp triangle.cpp
int main()
{
    auto t = RightAngledTriangle::with_a_b_c(1, sqrt(3), 2);
    assert(fcmp(t.A(), Angle::degrees(30)));
    assert(fcmp(t.B(), Angle::degrees(60)));
    assert(fcmp(t.a(), 1));
    assert(fcmp(t.b(), sqrt(3)));
    assert(fcmp(t.c(), 2));

    t = RightAngledTriangle::with_a_b_c(3, 4, 5);
    ...
    assert(RightAngledTriangle::with_a_b_c(3, 4, 5).description() == std::string("RightAngledTriangle 3, 4, 5; 0.643501 (36.8699°), 0.927295 (53.1301°)"));
}
